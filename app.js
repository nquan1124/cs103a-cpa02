...
app.get('/upsertDB',
  async (req,res,next) => {
    ...
    for (course of courses){
      const {author,country,language,pages,title,year}=course;
      const num = getNum(coursenum);
      await Course.findOneAndUpdate({author,country,language,pages,title,year},course,{upsert:true})
    }
...
app.post('/courses/byAuthor',
  async(req,res,next)=> {
    const {author} = req.body;
    var regex = new RegExp(author, "gi")
    const books = await Course.find({author:regex})
    res.locals.books = books
    res.render('courselist')
  }
)

pp.post('/courses/byPages',
  async(req,res,next)=> {
    const {pages} = req.body;
    var regex = new RegExp(pages, "gi")
    const books = await Course.find({pages:regex})
    res.locals.books = books
    res.render('courselist')
  }
)

pp.post('/courses/byTitle',
  async(req,res,next)=> {
    const {} = req.body;
    var regex = new RegExp(title, "gi")
    const books = await Course.find({title:regex})
    res.locals.books = books
    res.render('courselist')
  }
)
